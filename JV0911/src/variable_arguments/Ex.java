...
public class Ex {
	...
	public static void main(String[] args) {
		...
		NormalArgumnets na = new NormalArgumnets();
		na.print("홍길동");
		na.print("홍길동", "이순신");
		na.print("홍길동", "이순신", "강감찬");
//		na.print("홍길동", "이순신", "강감찬", "임소언"); // 오류 발생
		// -> 4개의 파라미터를 전달받는 메서드가 존재하지 않기 때문
		
		// 만약 배열을 파라미터로 사용하는 경우
		// 데이터 개수에 제한을 받지 않지만 배열을 미리 생성해야 한다!
		String[] strArr = {"홍길동", "이순신", "강감찬", "임소언"}; // 배열 생성
		na.print(strArr);
		
		System.out.println("----------------------------------");
		
		VariableArhuments va = new VariableArhuments();
		// VariableArhuments 객체(인스턴스)의 print() 메서드를 호출하면
...
class VariableArhuments {
	// 가변 인자를 사용한 메서드 오버로딩
	// -> 메서드 저으이 시 파라미터로 사용할 변수 데이터 타입 뒤에 ... 기호 붙임
	public void print(String... names) {
		// String 타입 데이터를 0~무한대로 전달받을 수 있으며
		// 전달된 데이터는 자동으로 배열 생성 후 저장됨
		// -> 가변인자를 사용한 변수 names는 Stirng[] 타입으로 사용됨
