...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 *          
		 * - 규칙2) int 타입보다 큰 타입과의 연산 큰 타입으로 변환 후 수행
		 *          -> 즉 결과값이 무조건 큰 타입이 됨
		 *          ex) int + long = (long)int + long = long + long = long
		 *          ex2) long + flaot = (float)long + float = float + float = float
		 *          
		 */
		...
		// b1 과 b2의 연산 결과를 byte 타입 변수 b3에 저장할 때 문제 발생
//		b3 = b1 + b2; // 오류 발생! byte + byte = int + int = int
		// -> Type mismatch: cannot convert from int to byte
		// -> 저장하려는 변수 데이터타입이 byte라서 문제가 발생!
...
//		b3 = (byte)b1 + (byte)b2; // 다시 각각 int로 변환되므로 또 오류 발생
	...
	//	short s = b1 + b2; // 오류 발생! byte + byte = int + int = int이므로
		// short 타입 변수에 s에 저장(자동 형변환) 불가!
		short s = (short)(b1 + b2);
		...
		// char 타입 변수 ch2에 ch값에 2를 더한 결과를 저장
...
//		int i2 = i + l; // int + long = long이므로 int 타입 변수에 저장 불가
		...
		// 3 /2 의 연산 결과는 정수 1이 출력됨
		// 만약 3/2 결과를 1.5 결과값으로 얻고 싶은 경우
		// 규칙 2를 활용!
		// 정수가 아닌 실수 형태의 나눗셈 연산으로 연산을 수행해야 함
		// 따라서, 최소한 피연산자 중 하나를 실수로 변환한 후 연산을 수행
		// -> 규칙2에 의해서 int형보다 큰 데이터 타입을 따라감
		...
		System.out.println(3 /(double)i2 );
