...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *   -> 따라서, 자바가 자동으로 형변환을 수행해 주지 않는다
		 *    (코드 상에 오류가 발생함)
		 * - 반드시 우변의 데이터 앞에 '형변환 연산자'를 사용하여
		 *   강제 형변환 수행해야 함(형변환 연산자에는 좌변의 작은 데이터타입 명시)
	...
	//	System.out.println("변환 전 a1(int): " + a1 + ", a2(short) : " + a2 );
		...
		//    명시적 형변환 후에도 Overflow 발생이 없으므로 원본 데이터가 유지
...
//		e1 = e2; // 오류 발생!
		...
		// -> 정수형 데이터타입 e1에는 소수점을 제외한 정수부분만 저장
		...
		long l = 100L;
		float f = l; // long < float, long -> float 변환시 자동 형변환 일어남
		
	//	l = f; // 오류 발생! float -> long 변환 시 자동 형변환이 불가!
