...
public class Ex {
	...
	public static void main(String[] args) {
		/*
		 * 중첩 클래스(Nested Class) - 클래스 내에서 정의한 클래스 - 중첩 클래스를 사용하면 두 클래스의 멤버들을 서로 쉽게 접근할 수
		 * 있고, 외부에서는 불필요한 관계 클래스를 감춤으로써 코드의 복잡성을 줄일 수 있는 장점 - 독립적인 일반 클래스 형태로 작성할 필요는
		 * 없지만 나름대로 클래스 형태(멤버변수, 메서드, 생성자)를 갖춰야 할 때 사용 - 보통, 자신의 클래스 내에서만 접근 가능하도록 전용
		 * 클래스로 정의할 때 사용 => 주로, GUI 구현 시 이벤트 처리를 위한 핸들러 클래스 정의 시 사용 - 외부클래스(Outer
		 * Class)와 내부클래스(Inner Class) 구분됨 - 내부클래스는 클래스 정의 위치에 따라 구분되며 클래스의 멤버로써 선언되는 중첩
		 * 클래스를 [멤버 클래스]라고 하며, 생성자 또는 메서드 내부에서 선언되는 중첩 클래스를 [로컬 클래스]라고 함 - 다시 멤버 클래스는
		 * 객체를 생성해야만 하는 1) 인스턴스 멤버 클래스와 클래스로 바로 접근할 수 있는 2) 정적 멤버 클래스로 구분할 수 있음
		 * 
		 * 
		 * 1) 인스턴스 멤버(내부) 클래스 - 멤버변수 및 메서드와 동일한 레벨에 정의한 클래스 - static 키워드를 지정하지 않은 클래스 -
		 * 인스턴스 멤버 변수(필드)와 메서드만 선언이 가능하고 정적 변수(필드)와 정적 메서드는 선언 불가 - 반드시 외부 클래스 인스턴스 생성 후
		 * 접근 가능
		 * 
		 * 2) 정적 멤버(내부) 클래스 - 멤버변수 및 메서드와 동일한 레벨에 정의한 클래스 - static 키워드를 지정한 클래스 - 모든 종류의
		 * 필드와 메서드를 선언 가능 - 외부 클래스명만으로 접근 가능
		 * 
		 * 3) 로컬 멤버(내부) 클래스 (=로컬 클래스) - 클래스 내의 메서드 내에서 정의한 클래스
		 * 
		 * - 멤버 클래스의 경우 [바깥클래스$멤버클래스.class] 파일 생성 로컬 클래스의 경우 [바깥클래스$1로컬클래스.class] 파일 생성
		 * 
		 * < 중첩 클래스 기본 문법 > class 바깥클래스명 { class 안쪽클래스명 { } }
		 */
...
//		Inner inner = new Inner();
		...
		//    따라서, 내부 클래스 접근 문법은 외부클래스 인스턴스 생성 없이
		//    외부 클래스 명만으로 접근 가능
